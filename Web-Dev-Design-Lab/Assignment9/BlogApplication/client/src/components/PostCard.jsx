...
const PostCard = ({ post }) => {
  ...
  return (
    <article className="group bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 ease-in-out flex flex-col h-full">
      {post.imageUrl && (
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="flex items-center text-sm text-slate-500 mb-4">
          <div className="h-6 w-6 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-bold text-xs mr-2 shrink-0">
            {post.author?.name?.charAt(0).toUpperCase() || 'A'}
          </div>
          <span className="font-medium text-slate-700 truncate">{post.author?.name}</span>
        ...
        <Link to={`/post/${post._id}`} className="block mb-4 flex-grow">
          <h2 className="text-xl font-bold font-poppins text-slate-900 mb-2 group-hover:text-indigo-600 transition-colors line-clamp-2 leading-snug">
            {post.title}
          </h2>
        ...
        <div className="flex items-center justify-between border-t border-slate-100 pt-4 mt-auto">
          <Link
            to={`/post/${post._id}`}
            className="text-indigo-600 text-sm font-semibold hover:text-indigo-700 flex items-center group/btn"
          >
            Read article 
